...
# draw projection of dots on horizontal plane
zMin = min(zLis) - (max(zLis)-min(zLis))/10
zMax = max(zLis)
zzLis = [zMin]*len(zLis)
ax.scatter(xLis, yLis, zzLis, marker='>', 
               c=[i/float(cnt) for i in range(cnt)],
               s = 50,
               lw = 0.2,
               cmap=plt.get_cmap('Spectral'))

# connect dots and their projection on horizontal plane
xs = zip(xLis, xLis)
ys = zip(yLis, yLis)
zs = zip(zLis, zzLis)
for xxs, yys, zzs in zip(xs, ys, zs):
    ax.plot(xxs, yys, zzs, '-.', c='0.6')
