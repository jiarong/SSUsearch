...
#TARGETS
all: tool_check diversity diversity_cc

.PHONY: ssusearch clust diversity diversity_cc clean very_clean setup tool_check
...
# use dummy as a hack
$(Dummyfiles_filter): %.$(Gene).out/dummy.qc.$(Gene).align.filter: %
	mkdir -p $(dir $@)
	make -f $(Script_dir)/ssusearch.Makefile $(Method) \
	  Seqfile=$< -C $(dir $@) Tag=dummy

region_cut: $(Dummyfiles_cut)

$(Dummyfiles_cut): %: %.qc.$(Gene).align.filter
	@echo
...
help:
	# ***This is a pipeline combining several tool. Thus there are many parameters, which can be changed in this Makefile for run as flag in command line (e.g. Make Seqfiles=Path_to_sequence_files)
	#
	# Parameters
	...
	# Gene_model_org: aligned gene sequence of a organism as standard for
	#                 gene position
