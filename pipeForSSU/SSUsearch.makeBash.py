...
for fName in sampleList:
    ...
    print >> fw, 'if [! -d %s]; then' %(outDir)
    print >> fw, '  echo "making %s.."' %(outDir)
    print >> fw, '  mkdir %s' %(outDir)
    print >> fw, 'else'
    print >> fw, '  echo "%s already exists.."' %(outDir)
    print >> fw, 'fi'
    ...
    #
    print >> fw, 'python /mnt/home/guojiaro/Documents/scripts/pipeForSSU/regionCut.py  %s/%s.afterMerge.SSU.align.badSeqsFiltered.RFadded 971 1118 100' %(outDir, sample)
    print >> fw, 'mv  %s/%s.afterMerge.SSU.align.badSeqsFiltered.RFadded.971to1118.cut.lenScreened.afa %s' %(outDir, sample, sample)
