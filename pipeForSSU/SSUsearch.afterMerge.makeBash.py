...
for fName in sampleList:
    ...
    print >> fw, 'time python /mnt/home/guojiaro/Documents/scripts/pipeForSSU/makeFastaForDNAHmmer.py %s' %(f)
    print >> fw, "echo 'reverse complement added for hmmsearch..'"


    print >> fw, "echo 'start hmmsearch'"
    print >> fw, 'time /mnt/home/guojiaro/Documents/software/hmmer3.0b3/./hmmsearch --incE 10 --incdomE 10 -A %s/%s.SSU.sto -o %s/%s.SSU.hmmout --tblout %s/%s.SSU.hmmtblout --domtblout %s/%s.SSU.hmmdomtblout /mnt/scratch/gjr/data/hmms/SSU.hmm %s/%s.RCaddedForHmmer' %(outDir, sample, outDir, sample, outDir, sample, outDir, sample, outDir, fName)
    ...
    #print >> fw, 'python /mnt/home/guojiaro/Documents/scripts/pipeForSSU/parseHMMgetSeq2.py %s/%s.SSU.hmmdomtblout %s/%s.RCaddedForHmmer %s/%s.SSU.fa' %(outDir, sample, outDir, sample, outDir, sample)
    print >> fw, 'rm  %s/%s.RCaddedForHmmer' %(outDir, fName)
    ...
    #
    print >> fw, 'python /mnt/home/guojiaro/Documents/scripts/pipeForSSU/regionCut.py  %s/%s.SSU.align.badSeqsFiltered.RFadded 971 1118 100' %(outDir, sample)
    print >> fw, 'mv  %s/%s.SSU.align.badSeqsFiltered.RFadded.971to1118.cut.lenScreened.afa %s' %(outDir, sample, sample)
