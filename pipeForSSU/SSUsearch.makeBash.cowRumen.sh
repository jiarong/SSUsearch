...
for fName in sampleList:
    ...
    print >> fw, '/mnt/home/guojiaro/Documents/software/flash_v1.0.2/./flash %s %s -m 10 -M 100 -x 0.20 -p 33 -o %s -r 125 -f 200 -s 20 -d %s' %(outDir+'/'+fName+'.1', outDir+'/'+fName+'.2', sample, outDir)
    ...
    print >> fw, 'time /mnt/home/guojiaro/Documents/software/mothur/Mothur.source/./mothur "#align.seqs(candidate=%s/%s.afterMerge.SSU.fa, template=/mnt/home/guojiaro/Documents/data/RefDB/silva.ssu.fasta, search=suffix, threshold=0.5, flip=t, processors=1)"' %(outDir, sample)
    print >> fw, 'python /mnt/home/guojiaro/Documents/scripts/pipeForSSU/mothurAlignReportParser.py  %s/%s.afterMerge.SSU.align.report  %s/%s.afterMerge.SSU.align' %(outDir, sample, outDir, sample)

    print >> fw, 'cat /mnt/home/guojiaro/Documents/data/16s_ecoli_J01695.afa  %s/%s.afterMerge.SSU.align.badSeqsFiltered > %s/%s.afterMerge.SSU.align.badSeqsFiltered.RFadded' %(outDir, sample, outDir, sample)
    #
    # pay att to the ave read len: 75, 100 , 125; minLen are 2/3 of ave len;    
    #
    print >> fw, 'python /mnt/home/guojiaro/Documents/scripts/pipeForSSU/regionCut.py  %s/%s.afterMerge.SSU.align.badSeqsFiltered.RFadded 971 1096 80' %(outDir, sample)
    print >> fw, 'mv  %s/%s.afterMerge.SSU.align.badSeqsFiltered.RFadded.971to1096.cut.lenScreened.afa %s' %(outDir, sample, sample)
